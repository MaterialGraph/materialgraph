...
| Recovery | Atomic checkpoint/resume and idempotent chunk replay implemented | Failed first chunk and stale-checkpoint committed replay verified with the exact real manifest | Define remote rollback and cleanup before isolated Neon qualification |
...
| Scale tests | Deterministic fixture and exact real cohort executed with the bounded sequential PostgreSQL harness | Locally qualified; heavy discovery paths and broad response crowding retained as signals | Repeat under isolated Neon telemetry; concurrency remains separately authorized |
...
not hidden successes. Isolated non-production Neon qualification is the next
