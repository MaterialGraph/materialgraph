...
The candidate `explanation` is currently a single backend-authored prose field. Some entries combine multiple composition heuristics and validation caveats into long semicolon-separated sentences. The explorer leads with the provided score factors and an explicit evidence limit, then makes the original backend prose inspectable on demand. It does not parse prose into new scientific claims. A future backend contract should expose distinct structured reasons, cautions, and evidence references for a better explanation summary.
