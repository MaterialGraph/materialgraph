...
| `MG-SA-O-002` | CI governance | Passing workflows are not enforced by branch protection/rulesets and push checks may be skipped | **Verification defect** | Claims and manual deployment precondition corrected, independently verified, and closed under `MG-SA-002`; absence of protected-branch enforcement remains the explicit `MG-SA-O-007` residual risk |
