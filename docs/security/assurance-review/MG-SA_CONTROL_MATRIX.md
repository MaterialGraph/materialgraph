...
**Assessment status:** `MG-SA-001` verified and closed; `MG-SA-002` correction implemented, verification pending
...
| `MG-SEC-010` | Dependency drift and vulnerable packages; exact hashed lock, clean install, reconciliation, `pip-audit` workflow | Validators parse hashes/inputs and reject malformed locks; current hosted run performed a real install/audit | Current SHA dependency job independently passed: contract, clean install, `pip check`, reconciliation, zero known vulnerabilities | Workflow is not required on `main` and can be skipped; corrected guidance requires exact-SHA Dependency Security and Secret Scan evidence as a manual production precondition | **Verification defect corrected in repository**; acceptance verification pending under `MG-SA-002` |
