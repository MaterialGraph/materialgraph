...
| Database transport | TLS is documented as required but not visibly enforced in SQLAlchemy configuration. | Inspect the live database session without exposing the connection string. |
| Database privilege | Application and migration role separation and grants are unverified. | Inspect redacted role attributes and grants. |
