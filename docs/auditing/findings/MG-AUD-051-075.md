...
**Resolution:** Discovery explanation construction now normalizes each
independently generated clause by trimming surrounding whitespace and adding
terminal punctuation when it is absent. Existing punctuation is preserved,
and the normalized clauses remain joined in their original order.

**Verification:** A focused regression reproduces the unpunctuated
evidence-limitation and score clauses and confirms that they are separated in
the final explanation. The 29 focused discovery tests passed, the full suite
passed with 640 tests, and Ruff passed for the affected service and test file.
...
**Resolution:** Removed the editable VCS self-dependency from
`requirements.txt`. Installation instructions now clone the canonical
`MaterialGraph/materialgraph` repository and explicitly install the current
checkout as an editable package. Configuration tests prevent another
MaterialGraph checkout from being reintroduced as a dependency.
...
**Resolution:** Setup, deployment, environment-template, Settings, and importer
semantics now consistently use `MATERIALS_PROJECT_API_KEY`. The importer error
names the actionable environment variable, and a regression test checks the
settings/documentation contract.
...
**Resolution:** `pyproject.toml` installed package metadata is the sole release
version authority. Runtime FastAPI metadata and health output use
`app.version.PROJECT_VERSION`; the configurable and hard-coded `1.0.0`
authorities were removed. Uninstalled source execution reports the non-release
fallback `0+unknown` rather than inventing another semantic version.
...
**Resolution:** Material import persistence now consumes the normalized,
deduplicated composition-fraction mapping produced by
`MaterialCompositionService.resolve_import_fractions` instead of iterating the
raw candidate element list. Each normalized symbol is therefore linked to a
material exactly once, while the existing legacy unknown-weight fallback is
preserved.

**Verification:** Regression coverage confirms whitespace-normalized duplicate
membership, legacy duplicate membership without explicit fractions, normalized
fraction resolution, and preservation of existing import validation behavior.
The 25 focused tests passed, the full suite passed with 592 tests, and Ruff
passed for all affected implementation and test files.
...
**Resolution:** Terminal updates now use atomic conditional database updates
that match both the job ID and the current `RUNNING` status. Only
`RUNNING -> COMPLETED` and `RUNNING -> FAILED` can succeed. Missing jobs,
pending jobs, terminal jobs, and repeated or contradictory terminal updates
return `None` without changing persisted state. Each successful transition
uses one timestamp for both `completed_at` and `updated_at`.

**Verification:** Service tests cover both permitted transitions; rejection
from `PENDING`; rejection after `COMPLETED` and `FAILED`; repeated terminal
updates; unknown job IDs; and timestamp consistency. A PostgreSQL concurrency
test confirmed that competing completion and failure attempts allow exactly
one terminal transition to succeed. The full regression suite passed with 545
tests, and Ruff passed for the changed service and test files against the
isolated `materialgraph_test` database. The temporary database override was
then removed, and the effective normal database was verified as
`materialgraph`.

**Scope boundary:** This closes contradictory graph-job lifecycle transitions.
The separate migration-downgrade issue that leaves the PostgreSQL `job_status`
enum behind remains an explicit follow-up.
...
Neighborhood nodes are still selected using the existing deterministic order,
but edges are now filtered against the IDs of the limited node set before the
edge limit is applied. Every returned edge therefore has both its source and
target in the same response. A one-node response contains only the root and no
edges, while sufficiently large limits retain all eligible edges. Reported
node and edge counts continue to match the returned collections.

Four focused service regressions verify graph closure under a restrictive
limit, the root-only boundary, retention under a large limit, and collection
count accuracy. The full regression suite passed with 549 tests, and Ruff
passed for the changed service and regression-test files.

Status: **Resolved and test-verified on 2026-08-11.**
...
**Resolution:** Similarity now selects the limited candidate set first and
bulk-loads criticality for the source material and those candidates through a
single `get_material_criticality_bulk()` call. Candidate processing reads from
the returned material-ID map, eliminating per-candidate criticality queries
while preserving unknown-criticality values, criticality deltas, directions,
and deterministic ranking behavior.

**Verification:** Sixteen focused similarity-service tests and the full
551-test regression suite passed. Ruff passed for the changed similarity
service and its test module.

Status: **Remediated and test-verified on 2026-08-13.**
