...
**Resolution:** Family relationships are now explicitly identified as
composition heuristics. Public results expose
`relationship_basis="composition_heuristic"` and
`structural_family_validated=false`, and explanations use candidate and
hypothesis language rather than claiming a validated structural family.
...
**Resolution:** A canonical periodic-group definition now treats only Li, Na,
K, Rb, Cs, and Fr as alkali metals. Mg is excluded, and family,
substitution-path, and transition-validation logic share that single domain
definition. Regression tests lock the taxonomy and prevent Mg from receiving
alkali-substitution semantics.
...
**Resolution:** Substitution paths and transitions now expose
`relationship_basis="composition_heuristic"` and
`substitution_mechanism_validated=false`. Graph edges expose
`plausibility_basis="internal_composition_heuristic"` with the same explicit
mechanism-validation flag. Explanations describe a composition-level
substitution hypothesis without claiming site equivalence, structural
compatibility, or a validated mechanism. Existing relationship identifiers
and numeric heuristic scores are preserved.

**Verification:** 44 focused tests and the 600-test full suite passed. Ruff
passed for all affected domain, service, schema, route, and test files.
