...
## Hop-bounded public discovery path lookup

Related finding: MG-AUD-084  
Date: 2026-07-31  
Release reference: Post-v1.9.18  
Status: Resolved; full-suite test-verified and endpoint smoke-tested 2026-07-31

### Before

The public path service accepted `max_hops` but always built a one-hop graph
and searched only for a direct source-to-target edge. Valid bounded multi-hop
paths were therefore omitted, and changing the parameter had no effect.

### After

Path lookup passes the requested bound into graph construction and uses
deterministic breadth-first search to return the shortest valid path within
that ceiling. Complete ordered material and transition sequences are returned,
cycles are prevented, and targets reachable only beyond the ceiling remain
not found.

Focused tests covered direct, two-hop, beyond-limit, deterministic
shortest-path, unreachable, cyclic, and response-continuity cases. The full
suite passed. The development graph contained no multi-hop-only pair among the
tested sources, so the differential hop-limit case remains fixture-verified.
A public `5 → 7` request with `max_hops=1` successfully returned a consistent
one-hop path.

### Impact

- Scientific result: **Yes; valid bounded multi-hop paths are no longer omitted**
- Ranking: **Potentially; the shortest newly reachable bounded path can now be ranked**
- API contract: **No structural change; the existing `max_hops` contract is now enforced**
- Data migration: **No**

---

## Endpoint-authoritative target-family evaluation

Related findings: MG-AUD-086, MG-AUD-087  
Date: 2026-07-31  
Release reference: Post-v1.9.18  
...
Target-family satisfaction and exploration scoring could use evidence from an
earlier transition rather than the material being evaluated. Scientific
pathway analysis also treated explicit `elements: []` as missing and reparsed
the endpoint formula.

### After

Chain filtering evaluates only the final endpoint, and exploration scoring
evaluates the candidate itself. Structured `elements` are authoritative
whenever not `null`, including an explicitly empty list; formula parsing occurs
only when membership is absent or `null`. Transition types and explanatory
prose no longer establish endpoint-family membership.

Focused regressions covered earlier-transition rejection, endpoint and
candidate composition matching, explicit-empty membership, and formula
fallback. The full regression suite and Ruff checks passed. Development smoke
tests for objective exploration and scientific pathways each returned five
valid phosphate endpoints whose formulas independently contained P and O.

### Impact

- Scientific result: **Yes; endpoint-family and coverage claims now require endpoint evidence**
